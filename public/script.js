...
function displayFilteredPointsOnMap(points) {
    // Clear existing filtered points markers
    if (window.filteredPointsMarkers) {
        window.filteredPointsMarkers.forEach(marker => marker.setMap(null));
    }
    window.filteredPointsMarkers = [];
    ...
    points.forEach(point => {
        if (
            ...
            google.maps.event.addListener(marker, 'mouseover', () => {
                document.getElementById('points-data-box').innerHTML = pointDataContent;
            });

            google.maps.event.addListener(marker, 'click', () => {
                document.getElementById('points-data-box').innerHTML = pointDataContent;
                window.filteredPointsMarkers.forEach(m => m._isSelected = false);
                marker._isSelected = true;
                const infoWindow = new google.maps.InfoWindow({ content: pointDataContent });
                infoWindow.open(map, marker);
            });
            ...
            marker.pointName = point.name;
            window.filteredPointsMarkers.push(marker);
        }
    });
...
// Handle filtered points upload
document.getElementById('uploadFilteredPointsBtn').addEventListener('click', async () => {
    const pointsFile = document.getElementById('filteredPointsFile').files[0];
    if (!pointsFile) {
        document.getElementById('result').innerHTML = '<p>Please select a KML file.</p>';
        return;
    }

    const formData = new FormData();
    formData.append('filteredPointsFile', pointsFile);

    const pointsLoading = document.getElementById('filtered-points-loading');
    const uploadPointsBtn = document.getElementById('uploadFilteredPointsBtn');
    pointsLoading.style.display = 'flex';
    uploadPointsBtn.disabled = true;

    try {
        console.log('Uploading filtered points...');
        const response = await fetch('/upload-filtered-points', {
            method: 'POST',
            body: formData,
        });
        if (!response.ok) throw new Error(`Server error: ${response.status}`);
        ...
        displayFilteredPointsOnMap(filteredPoints);

        document.getElementById('points-data-box').innerHTML = `
            <h3>Filtered Points (${filteredPoints.length})</h3>
            <table>
                ${filteredPoints.map(p => `
                    <tr>
                        <td>Name</td><td>${p.name}</td>
                    </tr>
                    <tr>
                        <td>Lat</td><td>${p.coordinates[1].toFixed(6)}</td>
                    </tr>
                    <tr>
                        <td>Lng</td><td>${p.coordinates[0].toFixed(6)}</td>
                    </tr>
                `).join('')}
            </table>
        `;
        document.getElementById('result').innerHTML = `<p>Loaded ${filteredPoints.length} filtered points.</p>`;
        document.getElementById('downloadFormat').disabled = false;
    } catch (error) {
        console.error('Filtered points upload error:', error);
        document.getElementById('result').innerHTML = `<p>Error uploading filtered points: ${error.message}</p>`;
    } finally {
        pointsLoading.style.display = 'none';
        uploadPointsBtn.disabled = false;
    }
...
function generateParsedKMLPoints(points) {
    if (!points || !points.length) {
        document.getElementById('result').innerHTML = '<p>No filtered points to download.</p>';
        return;
    }
  ...
  <Document>
    ...
    <Placemark>
      <name>${point.name || 'Unnamed'}</name>`;
            // Add properties as ExtendedData
    ...
    a.click();
    URL.revokeObjectURL(url);
    document.getElementById('result').innerHTML = '<p>Downloaded parsed_kml_points.kml</p>';
}