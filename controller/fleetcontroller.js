...
async function getAllExternalFilesWithPreview(req, res) {
  let connection;
  try {
    const { st_name, dt_name, blk_name } = req.query; // ?? use query params for filters

    connection = await pool.getConnection();

    // 1?? Build conditions dynamically
    const conditions = [];
    const params = [];
    ...
    // 2?? Get all networks with optional filters
    const [networks] = await connection.query(
      `SELECT * FROM networks ${whereClause}`,
      params
    );

    // 3?? For each network, fetch points + connections
    const results = [];
...
async function getExternalFiles(req, res) {
    ...
    try {
        ...
        // 1?? Fetch all files with category 'BSNL_Cables' and optional filters
