...
async function surveyCountByUser(req, res) {
  ...
  try {
    ...
    // ?? Step 1: Fetch survey counts by user
    const [rows] = await connection.query(
      `
      SELECT
        u.id AS user_id,
    ...
    // ?? Step 2: Calculate total_kms from LIVELOCATION data
    for (const user of rows) {
      // Get all survey IDs for this user
      const [surveys] = await connection.query(
        ...
        `,
        from_date && to_date
          ? [user.user_id, from_date, to_date]
          : [user.user_id]
      );
      ...
      for (const survey of surveys) {
        const [locations] = await connection.query(
          ...
          FROM underground_survey_data
          WHERE event_type = 'LIVELOCATION'
            AND survey_id_int = ?
      ...
      user.total_kms = totalDistance.toFixed(2);
    }

    // ?? Step 3: Get total users for pagination
    ...
    // ?? Step 4: Send final formatted response
    res.json({
      success: true,
      data: rows.map((r) => ({
        user_id: r.user_id,
        ...
        completed_surveys: r.completed_surveys.toString(),
        pending_surveys: r.pending_surveys.toString(),
        rejected_surveys: r.rejected_surveys.toString(),
        total_surveys: r.total_surveys,
        total_kms: r.total_kms || "0.00",
      })),
      pagination: {
