...
async function getDepthdata(req, res) {
  ...
  try {
    ...
    WHERE (
        (cf.start_lgd = ? AND cf.end_lgd = ?)
        OR
        (cf.start_lgd = ? AND cf.end_lgd = ?)
    )
`;

    const params = [start_lgd, end_lgd, end_lgd, start_lgd];
  ...
  try {
    ...
    if (!serial_number || !firm_name || !registration_number) {
      return res.status(400).send({
        status: false,
        message:
          "Missing required fields: serial_number, firm_name, registration_number",
      });
    }
...
async function loginUser(req, res) {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(401)
        .send({ status: false, message: "Invalid user_id or password" });
    }
...
async function createsurvey(req, res) {
  ...
  try {
    ...
    // 1?? Insert into underground_fiber_surveys
    ...
    // 2?? Insert vehicle info into construction_forms
    ...
    if (construction_type === "Hdd" || construction_type === "OpenTrench") {
      ...
      console.log(
        "Construction type is not HDD or OpenTrench, skipping vehicle info insertion",
      );
    }
    await connection.commit();
...
async function createEvent(req, res) {
  ...
  try {
    const allowedFields = [
      ...
      "manholeLatLong",
      "manholePhotos",
      "routeIndicatorLatLong",
      "routeIndicatorPhotos",
      "landmarkLatLong",
      "landmarkPhotos",
      "fiberTurnLatLong",
      "fiberTurnPhotos",
      "kilometerstoneLatLong",
      "kilometerstonePhotos",
      "status",
      "start_lgd",
      "end_lgd",
      "machine_id",
    ...
    for (const field of [...allowedFields, "link_name"]) {
      if (body[field] !== undefined) {
        columns.push(field);
        const value =
          Array.isArray(body[field]) || typeof body[field] === "object"
    ...
    if (columns.length === 0) {
      return res
        .status(400)
        .json({ status: false, message: "No valid data provided." });
    }
...
async function getLatestMachineActivity(req, res) {
  ...
  try {
    ...
    const [machines] = await connection.query(
      `SELECT machine_id, registration_number, authorised_person FROM Hdd_machines`,
    );
    ...
    return res.status(200).send({
      ...
      message:
        result.affectedRows > 0
          ...
          : "No machine found with this machine_id",
    });
  } catch (error) {
    if (connection) await connection.rollback();
...
async function deleteMachine(req, res) {
  ...
  try {
    ...
    return res.status(200).send({
      ...
      message:
        result.affectedRows > 0
          ? "Machine marked as inactive successfully"
          : "No machine found with this machine_id",
    });
  } catch (error) {
    if (connection) await connection.rollback();
...
async function getMachineDailyDistances(req, res) {
  ...
  try {
    ...
    if (!machine_id) {
      return res
        .status(400)
        .send({ status: false, error: "Missing machine_id" });
    }
    ...
    for (const date in groupedByDate) {
      ...
      for (let i = 0; i < coords.length - 1; i++) {
        try {
          ...
          console.warn(
            "Skipping invalid coordinate pair:",
            coords[i],
            coords[i + 1],
          );
          continue;
  ...
  try {
    ...
    const validatedMonth =
      month && !isNaN(month) && month >= 1 && month <= 12
        ...
        : defaultMonth;
    const validatedYear =
      year && !isNaN(year) && year >= 2000 ? parseInt(year) : defaultYear;
    ...
    for (const row of rows) {
      ...
      if (!groupedByMachineAndDate[machine_id])
        groupedByMachineAndDate[machine_id] = {};
      if (!groupedByMachineAndDate[machine_id][date])
        groupedByMachineAndDate[machine_id][date] = [];
    ...
    for (const machineId in groupedByMachineAndDate) {
      ...
      for (const date in groupedByMachineAndDate[machineId]) {
        ...
        const difference = parseFloat(
          (dailyDistance - dailyBaseline).toFixed(2),
        );
      ...
      if (monthlyTotalDistance < monthlyBaseline) {
        ...
        monthlyPenalty =
          monthlyTotalDistance < 5 ? segments * 42000 : segments * 40000;
        ...
        monthlyIncentive =
          monthlyTotalDistance <= 10 ? segments * 42000 : segments * 45000;
      }
...
async function getMachinesByFirm(req, res) {
  ...
  try {
    ...
    if (!firm_name) {
      return res
        .status(400)
        .json({ status: false, error: "Missing firm_name" });
    }
...
async function editUndergroundFiberSurvey(req, res) {
  ...
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ status: false, error: "Missing id in request body" });
    }
    ...
    if (updates.length === 0) {
      return res
        .status(400)
        .json({ status: false, error: "No valid fields provided to update" });
    }
...
async function getUndergroundSurvey(req, res) {
  ...
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ status: false, error: "Missing id in request params" });
    }
...
module.exports = {
  ...
  createsurvey,
  createEvent,
  getLatestMachineActivity,
  getFilteredSurveys,
  updateMachine,
  deleteMachine,
  getMachineDailyDistances,
  editImages,
  getSurveyHistoryByUser,
  getUndergroundSurvey,
  getMachineMonthlyAmount,
  editphysicalsurvey,
  getAllFirmNames,
  getMachinesByFirm,
  editUndergroundFiberSurvey,
};
