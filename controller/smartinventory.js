...
async function getRectificationSurveysByLocation(req, res) {
    ...
    try {
        ...
        // ? Fetch RECTIFICATION survey data only
        const [rows] = await connection.query(`
            SELECT 
                usd.*,
                ufs.state_id,
...
//-------------------------------arieal survey----------------------------------------------
async function  deleteAerialSurvey (req, res) {
  const { id } = req.params;   // aerial_fiber_survey.id
  let connection;
  ...
  try {
    ...
    // 1️⃣ Delete road crossings first
    await connection.query(
      `DELETE FROM aerial_road_crossing_models WHERE survey_id = ?`,
      [id]
    );

    // 2️⃣ Delete pole models
    await connection.query(
      `DELETE FROM aerial_pole_model WHERE survey_id = ?`,
      [id]
    );

    // 👉 Add more child tables here if needed

    // 3️⃣ Delete main survey
    const [result] = await connection.query(
      `DELETE FROM aerial_fiber_survey WHERE id = ?`,
      [id]
    );
...
// UPDATE BY TYPE + ID
async function updateAerialByType  (req, res) {
  ...
  if (type === "aerial") table = "aerial_fiber_survey";
  else if (type === "pole") table = "aerial_pole_model";
  else if (type === "roadcrossing") table = "aerial_road_crossing_models";
  else {
    return res.status(400).json({
      success: false,
      message: "Invalid type. Use: aerial | pole | roadcrossing",
    });
  }
  ...
  try {
    ...
    // convert photo arrays if coming from frontend
    if (Array.isArray(data.startGpPhotos))
      data.startGpPhotos = JSON.stringify(data.startGpPhotos);
    if (Array.isArray(data.endGpPhotos))
      data.endGpPhotos = JSON.stringify(data.endGpPhotos);
...
module.exports = {deleteAerialSurvey, updateAerialByType, getRectificationSurveysByLocation, getSurveysByLocation, uploadExternalData, getEXternalfiles, previewfile, insertFpoi, getGpsListPaginated, filterGpsList, updateGpsEntry, parseKmz, downloadshape, downloadExcel, getdesktopPlanning, deleteExternalFile }
